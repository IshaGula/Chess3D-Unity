...
public class MenuUiController : MonoBehaviour{
    ...
    private void OnStartClicked(){
        ...
        SceneLoading
            .Context
            .Inject(SceneLoading.Parameters.WhiteChoice,
            WhiteChoice.options[WhiteChoice.value].text);
        SceneLoading
            .Context
            .Inject(SceneLoading.Parameters.BlackChoice,
            BlackChoice.options[BlackChoice.value].text);
        SceneLoading
            .Context
            .Inject(SceneLoading.Parameters.WhiteDepth,
            Convert.ToInt32(WhiteDepth.options[WhiteDepth.value].text));
        SceneLoading
            .Context
            .Inject(SceneLoading.Parameters.BlackDepth,
            Convert.ToInt32(BlackDepth.options[BlackDepth.value].text));
        SceneLoading.Context.Inject(SceneLoading.Parameters.Fen, Fen.text);
        try{
            Board board;
            if (!string.IsNullOrEmpty(Fen.text))
                board = new Board(Fen.text);
            else
